...
import { LayoutDashboard, Calendar, FileText, Layers, Plus, HelpCircle, ChevronLeft, ChevronRight } from './Icons';
...
export const Layout: React.FC<LayoutProps> = ({
  ...
  return (
    <div className="relative h-full w-full flex flex-col md:flex-row overflow-hidden transition-all duration-700">
      ...
      <aside className={`hidden md:flex relative z-20 flex-col volumetric-surface rounded-[28px] m-4 mr-0 h-[calc(100vh-2rem)] transition-all duration-500 ease-smooth ${
        isSidebarCollapsed ? 'w-[96px] py-6 px-3 items-center' : 'w-80 p-6'
      ...
      <nav className={`md:hidden fixed bottom-6 left-4 right-4 z-40 volumetric-surface rounded-[32px] pb-safe px-2 transition-all duration-700 ${isFocusMode ? 'opacity-40 blur-sm hover:opacity-100 hover:blur-none grayscale' : 'opacity-100'}`}>
        <div className="flex justify-around items-center h-[72px]">
          ...
          <button
            onClick={onAddNew}
            className="volumetric-btn volumetric-btn-primary w-14 h-14 rounded-[22px] flex items-center justify-center text-theme-primary -mt-6 shadow-lg"
...
const NavButton = ({ active, onClick, icon, label, badge }: { 
  ...
  <button 
    onClick={onClick}
    className={`flex flex-col items-center justify-center gap-1 w-14 h-14 rounded-[22px] transition-all duration-300 ${
      active 
        ? 'text-theme-primary' 
  ...
  >
    ...
    <span className={`text-[9px] font-semibold tracking-wide transition-colors ${
      active ? 'text-theme-primary' : 'text-theme-tertiary'
    }`}>
      {label}
    </span>
  </button>
...
const SidebarButton = ({ active, onClick, icon, label, shortcut, badge, collapsed }: { active: boolean, onClick: () => void, icon: React.ReactNode, label: string, shortcut?: string, badge?: number, collapsed?: boolean }) => (
  <button 
    ...
    className={`flex items-center gap-4 transition-all duration-300 group relative ${
      collapsed ? 'w-14 h-14 mx-auto justify-center rounded-[20px]' : 'w-full px-5 py-4 rounded-[24px]'
    } ${
      active 
    ...
    {!collapsed && (
      <>
        ...
        {badge && badge > 0 ? (
          ...
          <span className={`ml-auto text-[10px] font-medium tracking-widest transition-opacity shrink-0 ${
